...
for move in range(len(lines)):
    ...
    print(stacks[from_idx -1])
    slices = len(stacks[from_idx - 1]) - number_moves
    val = stacks[from_idx - 1][slices:]
    stacks[from_idx - 1] = stacks[from_idx - 1][:slices]
    stacks[to_idx - 1].extend(val)

    #print(f"moving{val}")
    #print(f"{stacks[to_idx - 1]=}")
    #print(stacks[from_idx -1])
    #input()
